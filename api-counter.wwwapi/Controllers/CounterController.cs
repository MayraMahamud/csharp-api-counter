...
namespace api_counter.wwwapi.Controllers
{
    ...
    public class CounterController : ControllerBase
    {
        ...
        public async Task<IResult> Get(int value, int number)
        {
        ...
        [HttpGet]
        [Route("greaterthan/{number}")]
        public async Task<IResult> Get2(int value, int number)
        {
           var counter = counters.Where(c=>c.Value < number).ToList();  
        ...
        [HttpGet]
        [Route("increase/{value}")]
        public async Task <IResult> ExTest(int id) 
        { 
            var counter = counters.FirstOrDefault(c=> c.Id == id);
        ...
        [HttpGet]
        [Route("decrease/{value}")]
        public async Task<IResult> ExTest2(int id)
        {
            var counter = counters.FirstOrDefault(c => c.Id == id);

            if (counter == null)
            {
                 Console.WriteLine($" Counter {id} not found");
            }
            else
            {
                counter.Value--;
